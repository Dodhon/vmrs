...
1. **New or existing skill?**
   - If existing: Which skill to update? (list relevant skills from `~/.claude/skills/` and `.claude/skills/`)
   - If new: What should it be named?

2. **Storage location** (for new skills):
   - Personal (`~/.claude/skills/`) — available across all projects
   - Project (`.claude/skills/`) — shared with the repository
...
**Learning is too small for a skill**: Suggest creating a Claude rule (`.claude/rules/`) instead, which is better suited for single-line or short guidelines.
